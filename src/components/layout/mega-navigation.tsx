...
export function MegaNavigation({ className, itemClassName }: MegaNavigationProps) {
  const [activeId, setActiveId] = useState<string | null>(null);
  const activeSection =
    megaMenuSections.find((section) => section.id === activeId) ?? megaMenuSections[0];
  ...
  return (
    <div
      ...
      onMouseEnter={() => setActiveId((current) => current ?? megaMenuSections[0].id)}
    ...
    >
      <nav className="flex items-center gap-5 xl:gap-8" aria-label="Primary">
        {megaMenuSections.map((section) => (
          <a
            className={cn(
              "border-b border-transparent py-8 font-heading text-xs font-light uppercase tracking-[0.14em] transition-colors duration-smooth ease-luxury hover:border-current hover:text-current",
              activeId === section.id && "border-current text-current",
              itemClassName,
            )}
            href={section.href}
            key={section.id}
                ...
                >
                  ...
                  <div className="grid gap-3 p-5">
                    ...
                    <div className="flex items-end justify-between gap-4">
                      <h2 className="font-heading text-3xl font-light">{activeSection.label}</h2>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {activeSection.columns.map((column, columnIndex) => (
                    ...
                    >
                      <h3 className="font-heading text-xl font-light">{column.title}</h3>
